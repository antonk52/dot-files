...
function M.on_attach(_, bufnr)
    ...
    local function keymap(from, to)
        vim.keymap.set('n', from, to, {buffer = bufnr or 0, silent = true})
    end

    keymap('gD', vim.lsp.buf.declaration)
    keymap('gd', vim.lsp.buf.definition)
    keymap('K', vim.lsp.buf.hover)
    keymap('<leader>t', vim.lsp.buf.hover)
    ...
    keymap('gK', vim.lsp.buf.type_definition)
    keymap('<leader>rn', vim.lsp.buf.rename)
    keymap('<leader>ca', vim.lsp.buf.code_action)
    keymap('gr', vim.lsp.buf.references)
    keymap('<leader>L', M.show_current_line_dignostics)
    keymap('<leader>[', vim.diagnostic.goto_prev)
    keymap('<leader>]', vim.diagnostic.goto_next)
    keymap('<localleader>f', vim.lsp.buf.formatting)
end
